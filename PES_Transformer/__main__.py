"""
PES_Transformer — Main entry point for the Pandemic Experiment Scenario (Transformer).

Orchestrates the full experiment lifecycle: configuration validation,
Transformer model loading (causal actor-critic policy), experiment execution
across blocks/sequences/trials, result formatting, and log management.
...
from . import (AGGREGATION_METHOD, ANSI, AVAILABLE_RESOURCES_PER_SEQUENCE,
               INIT_NO_OF_CITIES, INITIAL_SEVERITY_FILE, INPUTS_PATH,
               ...
               NUM_ATTEMPTS_TO_ASSIGN_SEQ, NUM_BLOCKS, NUM_MAX_TRIALS,
               NUM_MIN_TRIALS, NUM_SEQUENCES, OUTPUT_FILE_PREFIX, OUTPUTS_PATH,
               PLAYER_TYPE, RANDOM_INITIAL_SEVERITY, SAVE_INITIAL_SEVERITY_TO_FILE,
               SAVE_RESULTS, STARTING_BLOCK_INDEX, STARTING_SEQ_INDEX,
               TOTAL_NUM_TRIALS_IN_BLOCK, TRUST_MAX, USE_FIXED_BLOCK_SEQUENCES)
...
def main():
    """Run the PES_Transformer experiment lifecycle."""

    global Responses_filehandle

    # Validate Transformer model files if needed
    if PLAYER_TYPE == 'RL_AGENT':
        ...
        terminal_utils.header("TRANSFORMER-AGENT VALIDATION", width=80)
        terminal_utils.info("Checking Transformer model files...")

        if not os.path.isfile( config_file ):
            terminal_utils.error("Transformer config file not found!")
            terminal_utils.list_item(f"Expected path: {config_file}", level=2)
            print("\nTo train the Transformer agent, run:")
            terminal_utils.list_item("python3 -m PES_Transformer.ext.train_transformer")
            print()
            return

        if not os.path.isfile( weights_file ):
            terminal_utils.error("Transformer weights file not found!")
            terminal_utils.list_item(f"Expected path: {weights_file}", level=2)
            print("\nTo train the Transformer agent, run:")
            terminal_utils.list_item("python3 -m PES_Transformer.ext.train_transformer")
            print()
            return
        ...
        try:
            ...
            terminal_utils.success("Transformer config loaded successfully")
            terminal_utils.list_item(f"d_model: {model_config.get('d_model', '?')}", level=2)
            terminal_utils.list_item(f"n_layers: {model_config.get('n_layers', '?')}", level=2)
            terminal_utils.list_item(f"n_heads: {model_config.get('n_heads', '?')}", level=2)

            # Check weights file is non-empty
            weights_size = os.path.getsize( weights_file )
            terminal_utils.success("Transformer weights file found")
            terminal_utils.list_item(f"Size: {weights_size / 1024:.1f} KB", level=2)
            print()
        except Exception as e:
            terminal_utils.error("Failed to load Transformer model files!")
        ...
        terminal_utils.success("All Transformer model files validated successfully!")
        print()

    # --------------------------------- #
    #       Files to be written         #
    # --------------------------------- #
    # Pre-initialize variables used after conditional blocks
    MyPerformances: list  = []
    MySubjectId: str = ""
    session_outputs_path: str = ""
    total_movement: dict = {}
    MyMessage: numpy.ndarray = numpy.empty(0)
    ...
    if SAVE_RESULTS:
        exp_utils.exit_experiment_gracefully(
            "",
            [Responses_filehandle],
            (os.path.join(session_outputs_path, f'{OUTPUT_FILE_PREFIX}movement_log_{MySubjectId}.npy'), total_movement),
            log_utils,
            pygameMediator
        )
