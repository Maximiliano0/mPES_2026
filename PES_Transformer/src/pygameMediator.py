"""PES_Transformer — RL Agent Response Handler and Confidence Estimator.
...
def provide_rl_agent_response(
    ...
    Notes
    -----
    - Requires pre-training via: ``python3 -m PES_Transformer.ext.train_transformer``
    ...
    if _transformer_model is None:
        ...
        if not os.path.isfile(config_file) or not os.path.isfile(weights_file):
            raise FileNotFoundError(
                ...
                f"Please train the agent first by running: python3 -m PES_Transformer.ext.train_transformer\n"
            )
