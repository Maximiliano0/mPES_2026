'''
pes_ac - Pandemic Experiment Scenario
...
The evaluation uses ``ac_agent_meta_cognitive`` to compute entropy-based
confidence from the Actor's π(a|s) distribution.  The combined objective is::
...
Usage:
    python3 -m pes_ac.ext.optimize_ac [n_trials] [--resume YYYY-MM-DD]
...
##  Imports internos    ##
##########################
from .pandemic import Pandemic, run_experiment, A2CTraining, ac_agent_meta_cognitive
...
def objective(trial: optuna.Trial) -> float:
    """Train an A2C agent with sampled hyperparameters and return combined objective.
    ...
    64 fixed sequences using :func:`ac_agent_meta_cognitive` for entropy-based
    confidence, and returns a combined metric::
    ...
    def qf(_env, state, _seqid):
        ...
        response, confidence, _rt_hold, _rt_release = ac_agent_meta_cognitive(
            probs, state[0], 10000
        )
        if state[0] == 0:
            confidence = -1.0
...
def _save_report(study, opt_dir, opt_date, best_model, best_rewards):
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        ...
        f.write("-" * 100 + "\n")
        f.write(
            f"{'#':>4s}  {'objective':>10s}  {'perf':>10s}  {'conf':>6s}  {'a_lr':>10s}  "
    ...
    ax.set_title('Bayesian Optimisation: Convergence — A2C (meta-cognitive)', fontsize=14, fontweight='bold', pad=20)
...
def main():
    ...
    study = optuna.create_study(
        direction='maximize',
        study_name=f'a2c_metacog_{opt_date}',
    ...
    def _progress_callback(study, trial):
        ...
        if done > 0 and done % 10 == 0:
            notify(
                ...
                f"Mejor objetivo hasta ahora: {best_val:.6f}\n"
