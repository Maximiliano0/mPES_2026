...
def objective(trial: optuna.Trial) -> float:
    ...
    64 fixed sequences with infeasible-action masking, and returns the mean
    ...
    def qf(_env, state, _seqid):
        ...
        # Mask infeasible actions (consistent with ac_agent_meta_cognitive in __main__)
        options = probs.copy()
...
def _save_report(study, opt_dir, opt_date, best_model, best_rewards):
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        ...
        f.write("-" * 100 + "\n")
        f.write(
            f"{'#':>4s}  {'objective':>10s}  {'perf':>10s}  {'a_lr':>10s}  "
