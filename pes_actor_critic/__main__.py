"""
Main entry point for the pes_actor_critic experiment.
...
via ext/train_ac.py; the resulting Keras Actor model is consumed here at
experiment time.
...
def main():
    """Orchestrate the full pes_actor_critic experiment lifecycle."""

    global Responses_filehandle

    # Validate A2C Actor model and rewards file if needed
    if PLAYER_TYPE in ('RL_AGENT', 'DQN_AGENT', 'AC_AGENT'):
        ...
        if not os.path.exists(model_file):
            ...
            terminal_utils.list_item("python3 -m pes_actor_critic.ext.train_ac")
            print()
            return
        ...
        for CurrentBlockIndex in range(NUM_BLOCKS):
            ...
            for CurrentSequenceIndex, CurrentSequenceMapIndex in enumerate(CurrentBlockMapIndices):
                ...
                for trial_no in range(
                    ...
                    if resources_left > 0 and PLAYER_TYPE in ('RL_AGENT', 'DQN_AGENT', 'AC_AGENT'):
                        (pc,
                         r,
                         rt_h,
                         rt_rel,
    ...
    if PLAYER_TYPE in ('RL_AGENT', 'DQN_AGENT', 'AC_AGENT') and len(MyPerformances) > 0:
        try:
