"""
Configuration file for the pes_actor_critic experiment.

Centralises all tunable experiment parameters in one place.
Values are imported by pes_actor_critic/__init__.py and re-exported at package level.
...
- Player & Agent Settings        (RL_AGENT / AC_AGENT selector)
...
AC_ACTOR_HIDDEN_UNITS: list[int] = [64, 64]     # Hidden layers for the Actor (policy) network
AC_CRITIC_HIDDEN_UNITS: list[int] = [64, 64]    # Hidden layers for the Critic (value) network
AC_ACTOR_LR = 3e-4                               # Adam learning rate for the Actor
AC_CRITIC_LR = 1e-3                              # Adam learning rate for the Critic
AC_ENTROPY_COEFF = 0.01                          # Entropy regularisation coefficient (exploration)
AC_DISCOUNT = 0.99                               # Discount factor γ for return computation
