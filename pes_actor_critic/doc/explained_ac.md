# Mapeo Teoría Actor-Critic (A2C) ↔ Implementación en pes_actor_critic

## 1. Introducción

Este documento conecta la **teoría de Advantage Actor-Critic (A2C)** con su
**implementación concreta** en el paquete `pes_actor_critic`.  Para cada
concepto teórico se indica la variable, función o línea de código
correspondiente.

`pes_actor_critic` reemplaza la red DQN de `pes_dqn` (y la tabla Q tabular
de `pes` / `pes_base_line`) por **dos redes neuronales separadas**:
...
| Aspecto | DQN (`pes_dqn`) | A2C (`pes_actor_critic`) |
|---------|-----------------|--------------------------|
...
En `pes_actor_critic`, la salida del Actor $\pi_\theta(a \mid s)$ **es** una
...
### 8.2 Parámetro `compute_confidence`
...
| `num_episodes` | $[50\,000,\; 1\,000\,000]$ | entero (paso 50k) |
| `actor_hidden_dim` | $[32,\; 64]$ | entero (paso 32) |
| `critic_hidden_dim` | $[32,\; 64]$ | entero (paso 32) |
| `n_hidden_layers` | $\{1, 2\}$ | categórico |
...
| Componente | `pes` (Q-tabular) | `pes_dqn` (DQN) | `pes_actor_critic` (A2C) |
|------------|-------------------|-----------------|--------------------------|
