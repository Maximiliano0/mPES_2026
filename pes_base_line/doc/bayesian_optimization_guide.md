...
Los resultados se guardan en `pes_base_line/inputs/<FECHA>_BAYESIAN_OPT/`.
...
pgrep -f "pes_base_line.ext.optimize_rl"
...
```bash
nohup python3 -m pes_base_line.ext.optimize_rl 100 --resume 2026-02-12 \
  > pes_base_line/inputs/bayesian_opt_resume.log 2>&1 &
```
...
```bash
grep "Trial" pes_base_line/inputs/bayesian_opt.log | tail -10
```
...
```bash
tail -f pes_base_line/inputs/bayesian_opt.log
```

### Verificar que el proceso sigue vivo

```bash
pgrep -f "pes_base_line.ext.optimize_rl" -a
```
...
```bash
tail -5 pes_base_line/inputs/bayesian_opt.log
```
...
```bash
python3 -m pes_base_line.ext.train_rl
```
