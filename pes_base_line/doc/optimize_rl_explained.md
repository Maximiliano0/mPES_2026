...
experimento con `python3 -m pes_base_line`):
...
sea **consistente** con el rendimiento observado al ejecutar `python3 -m pes_base_line`.
...
El módulo `pes_base_line` configura `numpy.seterr(all='raise', under='ignore')` en
...
python3 -m pes_base_line.ext.optimize_rl

# 100 trials
python3 -m pes_base_line.ext.optimize_rl 100
```

### 4.2 Ejecución en segundo plano (recomendada)

```bash
nohup python3 -m pes_base_line.ext.optimize_rl 100 \
  > pes_base_line/inputs/bayesian_opt.log 2>&1 &
```
...
```bash
python3 -m pes_base_line.ext.optimize_rl 100 --resume 2026-02-12
```
...
Todos se guardan en `pes_base_line/inputs/<FECHA>_BAYESIAN_OPT/`:
...
Para que `python3 -m pes_base_line` use la Q-table entrenada, ésta debe copiarse
a la raíz de `inputs/`:

```bash
cp pes_base_line/inputs/<fecha>_RL_TRAIN/q_<fecha>.npy  pes_base_line/inputs/q.npy
cp pes_base_line/inputs/<fecha>_RL_TRAIN/rewards_<fecha>.npy  pes_base_line/inputs/rewards.npy
```
