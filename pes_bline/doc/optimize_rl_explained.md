...
experimento con `python3 -m pes_bline`):
...
sea **consistente** con el rendimiento observado al ejecutar `python3 -m pes_bline`.
...
El módulo `pes_bline` configura `numpy.seterr(all='raise', under='ignore')` en
...
```bash
python3 -m pes_bline.ext.optimize_rl 100 --resume 2026-02-12
```
...
Todos se guardan en `pes_bline/inputs/<FECHA>_BAYESIAN_OPT/`:
...
Para que `python3 -m pes_bline` use la Q-table entrenada, ésta debe copiarse
a la raíz de `inputs/`:
