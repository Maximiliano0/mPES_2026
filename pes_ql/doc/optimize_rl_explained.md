...
sea **consistente** con el rendimiento observado al ejecutar `python3 -m pes_ql`.
...
Para que `python3 -m pes_ql` use la Q-table entrenada, ésta debe copiarse
a la raíz de `inputs/`:

```bash
cp pes_ql/inputs/<fecha>_RL_TRAIN/q_<fecha>.npy  pes_ql/inputs/q.npy
cp pes_ql/inputs/<fecha>_RL_TRAIN/rewards_<fecha>.npy  pes_ql/inputs/rewards.npy
```
