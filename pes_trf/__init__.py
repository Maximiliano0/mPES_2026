"""
pes_trf — Pandemic Experiment Scenario (Transformer)

Package initialization module for the pes_trf project, a variant of
pes_base that replaces tabular Q-Learning with a causal Transformer encoder and
actor-critic output heads.  The model processes the trajectory of states
within an episode (max 10 timesteps) and outputs action logits + value
estimates via multi-head causal self-attention.
...
Train Transformer agent:   ``python3 -m pes_trf.ext.train_transformer [n_batches]``
"""
######################
## External Imports ##
######################
