...
if (-not $PkgName) {
    Write-Error "Paquete desconocido: '$Package'. Opciones: bayesian, dql, dqn, ac, transformer"
    exit 1
}
...
if (Test-Path $Watcher) {
    # NOTE: Do NOT use -RedirectStandardOutput/-Error here.
    # Those flags force CreateProcess (child process inside VS Code's job group)
    # instead of ShellExecute (independent process that survives VS Code close).
    # The watcher handles its own logging via Start-Transcript (-LogFile param).
    $watchProc = Start-Process -FilePath powershell -ArgumentList @(
        '-NoProfile', '-ExecutionPolicy', 'Bypass', '-WindowStyle', 'Hidden',
        '-File', $Watcher, $PkgName, '-LogFile', $WatcherLog, "$OptPid"
    ) -WorkingDirectory $ProjectDir -PassThru -WindowStyle Hidden
    $watcherPid = $watchProc.Id
    # Save watcher PID
    $WatcherPidFile = Join-Path $LogDir "watcher${LogSuffix}.pid"
    "$watcherPid" | Set-Content -Path $WatcherPidFile -Encoding ASCII
